from tf_keras.layers import Conv2D, MaxPool2D, BatchNormalization
from tf_keras import Input, Model, Sequential
...
# Create generators Locally
#train_gen = BatchGenerator(train_tiles, '/home/emrys/PycharmProjects/PythonProject/LiDAR/lidar_chm', '/home/emrys/PycharmProjects/PythonProject/LiDAR/lidar_tag', batch_size=BATCH_SIZE)
#valid_gen = BatchGenerator(valid_tiles, '/home/emrys/PycharmProjects/PythonProject/LiDAR/lidar_chm', '/home/emrys/PycharmProjects/PythonProject/LiDAR/lidar_tag', batch_size=BATCH_SIZE)
...
# Define model
model = Sequential()
model.add(Input(shape=[1000, 1000, 1]))
...
#
# # First block
# # Note: padding='same' ensures that the output map has the same spatial dimensions as the input map
# model.add(Conv2D(filters=16, kernel_size=3, padding='same', activation='relu'))
# model.add(BatchNormalization())
# model.add(MaxPool2D(pool_size=2))  # Reduces dimensions by half (500x500)
#
# # Second block
# model.add(Conv2D(filters=32, kernel_size=3, padding='same', activation='relu'))
# model.add(BatchNormalization())
# model.add(MaxPool2D(pool_size=2))  # Reduces dimensions by half (250x250)
#
# # Third block
...
def weighted_loss(a, b):
    return weighted_cross_entropy_with_logits(a, b, pos_weight=50) # Increasing weight increases value of buildings
...
# lidar_chm, tag = train_gen.__getitem__(0)
# i = 4
# plt.imshow(lidar_chm[i])
# plt.show()
